...
The shared default lets a single copy of each dictionary be reused across tools. Missing dictionaries aren't fatal: the affected features are unavailable and `/health` reports the service as degraded until you fetch them and rebuild the cache.
