...
The store is portable plain SQLite and is designed to be extractable into its own service.
...
| `fuzzy` | One weighted cost blending unknown count (superlinear) with how good the new words are on average - each one's JPDB rarity and how little of the rest of the batch it unlocks - plus sentence length |

`greedy` never trades an extra unknown word for anything; `fuzzy` will, whenever the words it takes on are common enough and unlock enough of the rest of the batch to be worth the extra one. It judges the new words on their average quality rather than their total, so an extra word is only a penalty when it is a *worse* word - which is what lets two everyday, widely-used words beat one rare dead end. How readily it makes that trade is set by the weights at the top of `app/mining/ordering/fuzzy.py`.
