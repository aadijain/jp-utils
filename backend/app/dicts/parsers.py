...
class MeaningRow(NamedTuple):
    lemma: str
    reading: str
    senses: list[dict]  # [{"pos": [str], "glosses": [str], "examples": [{ja, en, segments}]}]
    score: int
    seq: int
    jlpt: int | None
...
def _ruby_segment(node: dict, keyword: bool) -> dict:
    """A `ruby` node -> one segment: its base text plus the `rt` reading."""
    base = ""
    reading = ""
    content = node.get("content")
    for child in content if isinstance(content, list) else [content]:
        if isinstance(child, str):
            ...
            if child.get("tag") == "rt":
                reading += _plain_text(child.get("content"))
            else:  # nested non-rt markup inside the ruby base (rare)
                base += _plain_text(child)
    return {"text": _clean_text(base), "reading": _clean_text(reading), "kw": keyword}


def _example_segments(node: Any, keyword: bool, out: list[dict]) -> None:
    """Flatten an `example-sentence-a` subtree into furigana + keyword segments.

    A run of the sentence is either plain text (``reading=""``) or a ruby base
    with its reading. ``kw`` marks segments inside an ``example-keyword`` span -
    jitendex's own highlight of the headword occurrence, so the target word is
    known without a locate call. Drops rt (folded into ruby), links, footnotes.
    """
    if isinstance(node, list):
        for n in node:
            _example_segments(n, keyword, out)
        return
    if isinstance(node, str):
        text = _clean_text(node)
        if text:
            out.append({"text": text, "reading": "", "kw": keyword})
        return
    if not isinstance(node, dict):
        return
    if node.get("type") == "image" or node.get("tag") in ("rt", "a"):
        return
    data = node.get("data")
    if isinstance(data, dict) and data.get("content") == "attribution-footnote":
        return
    if node.get("tag") == "ruby":
        out.append(_ruby_segment(node, keyword))
        return
    kw = keyword or (isinstance(data, dict) and data.get("content") == "example-keyword")
    if isinstance(node.get("text"), str):
        text = _clean_text(node["text"])
        if text:
            out.append({"text": text, "reading": "", "kw": kw})
        return
    if "content" in node:
        _example_segments(node["content"], kw, out)


def _merge_example_segments(segs: list[dict]) -> list[dict]:
    """Coalesce adjacent plain (no-reading) runs sharing a keyword flag."""
    out: list[dict] = []
    for s in segs:
        if s["reading"] == "" and out and out[-1]["reading"] == "" and out[-1]["kw"] == s["kw"]:
            out[-1] = {"text": out[-1]["text"] + s["text"], "reading": "", "kw": s["kw"]}
        else:
            out.append(s)
    return out


def _collect_examples(node: Any, out: list[dict]) -> None:
    ...
    translation (`example-sentence-b`). ``ja`` is the ruby-stripped plain text;
    ``segments`` preserve the source furigana + the ``example-keyword`` highlight.
    """
    if isinstance(node, list):
        for n in node:
    ...
    if _data_role(node) == "example-sentence":
        ...
        for child in content if isinstance(content, list) else [content]:
            ...
            if role == "example-sentence-a":
                ja_content = child.get("content")
                ja = _clean_text(_plain_text(ja_content))
            elif role == "example-sentence-b":
                en = _clean_text(_plain_text(child.get("content")))
        if ja and len(out) < MAX_EXAMPLES_PER_SENSE:
            segments: list[dict] = []
            _example_segments(ja_content, False, segments)
            out.append({"ja": ja, "en": en, "segments": _merge_example_segments(segments)})
        return
    if "content" in node:
