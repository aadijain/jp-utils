from app.mining.ordering import greedy_order
...
def test_greedy_chains_one_new_word_at_a_time():
    ...
    assert greedy_order(sentences, known=set(), ranks={}) == [1, 0, 2]
...
def test_known_set_counts_as_already_learnt():
    ...
    sentences = [["w1", "w2"], ["w2"], ["w1", "w3"]]
    order = greedy_order(sentences, known={"w1", "w3"}, ranks={})
...
def test_rank_tiebreak_prefers_more_frequent_unknown():
    ...
    order = greedy_order(sentences, known=set(), ranks={"rare": 9000, "common": 50})
    assert order == [1, 0]
...
def test_rank_tiebreak_uses_least_frequent_unknown():
    ...
    order = greedy_order([sentB, sentA], known=set(), ranks=ranks)
