...
def run_pipeline(
    ...
    def on_done(future) -> None:
        ...
        n_notes += m_notes
        n_fields += m_fields
        if on_applied is not None and (n_notes or n_cards or gen):
            on_applied()
        _report_done(parent, n_notes, n_fields, n_cards, gen, silent)
...
def _report_done(
    parent, n_notes: int, n_fields: int, n_cards: int, gen: _GenCounts, silent: bool
) -> None:
    parts = []
    if n_notes:
